...
Supports multiple variables; output under prism_data/<var>/<year>/<YYYYMMDD>/.
"""
...
class PRISMDownloaderParallel(BaseDownloader):
    ...
    def _daily_filename(self, variable: str, date: datetime) -> str:
        return f"prism_{variable}_us_25m_{date:%Y%m%d}.zip"

    def _daily_url(self, variable: str, date: datetime) -> str:
        return f"{self.base_url}/{variable}/daily/{date.year}/{self._daily_filename(variable, date)}"

    def _output_path(self, variable: str, date: datetime) -> Path:
        date_dir = self.output_dir / variable / str(date.year) / f"{date:%Y%m%d}"
        date_dir.mkdir(parents=True, exist_ok=True)
        return date_dir / "data.zip"

    def extract_zip(self, zip_file: Path, variable: str, date: datetime) -> Path:
        extract_dir = zip_file.parent
        with zipfile.ZipFile(zip_file, "r") as z:
            z.extractall(extract_dir)
        prefix = f"prism_{variable}_us_25m_{date:%Y%m%d}"
    ...
    def _download_one(self, task: PRISMTask) -> tuple[PRISMTask, str]:
        variable, date, extract = task.variable, task.date, task.extract
        url = self._daily_url(variable, date)
        out = self._output_path(variable, date)
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if extract:
                    self.extract_zip(out, variable, date)
    ...
    def download_date_range(
        ...
        for var in variables:
            for d in dates:
                tasks.append(PRISMTask(variable=var, date=d, extract=extract))
