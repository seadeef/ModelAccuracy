...
class GraphCastDownloaderParallel(BaseDownloader):
    ...
    def extract_forecast(
        ...
        if forecast_hours is None:
            forecast_hours = [d * 24 for d in range(1, GRAPHCAST_MAX_LEAD_H // 24 + 1)]
        if lead_windows is None:
            lead_windows = []

        if init_date is None:
            all_inits = sorted(gc_dir.glob(f"*/*_{GRAPHCAST_CYCLE:02d}z"), reverse=True)
            for candidate in all_inits:
                if any(candidate.glob("f*_*.npy")):
                    init_date = datetime.strptime(candidate.name[:8], "%Y%m%d")
                    break
            if init_date is None:
                raise SystemExit("No GraphCast init directories with data found.")
            print(f"Using most recent init date: {init_date.date()}")

        date_str = init_date.strftime("%Y%m%d")
        init_dir = gc_dir / str(init_date.year) / f"{date_str}_{GRAPHCAST_CYCLE:02d}z"
        if not init_dir.exists():
            raise SystemExit(f"Init directory not found: {init_dir}")

        grid_lats_path = gc_dir / "grid_lats.npy"
        grid_lons_path = gc_dir / "grid_lons.npy"
        ...
        print(f"Init date: {init_date.date()} {GRAPHCAST_CYCLE:02d}z")
